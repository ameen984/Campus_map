...
function setupGPSMarker() {
    ...
    // Cone for the "pin" - now using markerSize for scaling
    const coneGeometry = new THREE.ConeGeometry(3 * GPS_CONFIG.markerSize, 10 * GPS_CONFIG.markerSize, 16);
    ...
    cone.position.y = 5 * GPS_CONFIG.markerSize;  // Scale position with marker size
    cone.renderOrder = 999; // High render order to ensure it's drawn on top

    // Optional: add a small sphere on top to look like a pin head - now using markerSize
    const sphereGeometry = new THREE.SphereGeometry(2 * GPS_CONFIG.markerSize, 16, 16);
    ...
    sphere.position.y = 10 * GPS_CONFIG.markerSize;  // Scale position with marker size
    sphere.renderOrder = 999; // High render order to ensure it's drawn on top

    // Add a pulsing ring effect - now using markerSize
    const pulseGeometry = new THREE.RingGeometry(4 * GPS_CONFIG.markerSize, 5 * GPS_CONFIG.markerSize, 32);
    ...
    pulseRing.position.y = 0.1 * GPS_CONFIG.markerSize;  // Scale position with marker size
    ...
    // Make gpsMarker globally accessible for cameraFollowSystem
